...
4. 不要手写标题编号，网站会自动编号。
5. 表格必须使用 GFM Markdown 表格语法。
6. 每个表格的表题必须单独放在表格正上方一行，格式为“表 1 表题文字”或“表1 表题文字”；表题行和表格之间不要插入其他内容。
7. 如果网站开启表题自动编号，表题行也可以写成“表 表题文字”，网站会自动整理为“表1-1”或“表1”。
8. 图片必须单独成段，不要和正文放在同一行。
9. 每张图片的图题必须单独放在图片正下方一行，格式为“图 图题文字”或“图1 图题文字”；图题行和图片之间不要插入其他内容。
10. 如果网站开启图题自动编号，图题会自动整理为“图1-1”或“图1”，不要在图题正文里重复编号。
11. 表格内容只写数据，不要在表格内部写表题。
12. 正文段落保持自然中文论文表达，避免使用 HTML。
13. 如果需要代码块，使用 fenced code block。
